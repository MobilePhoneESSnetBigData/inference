...
#' @include utils.R
#' @export  
computeInitialPopulationBNB <- function(nnet, params, rndVal = FALSE, ciprob = NULL, method = 'ETI') {
    ...
    N_bnb[,row:=NULL]    
    pmean <- N_bnb[, .SD[, round(mean(NPop))], by = 'region']
    pmode <- N_bnb[, .SD[, round(Mode(NPop))], by = 'region']
    pmedian <- N_bnb[, .SD[, round(median(NPop),2)], by = 'region']

    pmin<- N_bnb[, .SD[, round(min(NPop))], by = 'region']
    pmax<- N_bnb[, .SD[, round(max(NPop))], by = 'region']
    pq1 <- N_bnb[, .SD[, round(as.numeric(quantile(NPop)[2]))], by = 'region']
    pq3 <- N_bnb[, .SD[, round(as.numeric(quantile(NPop)[4]))], by = 'region']
    piqr <- N_bnb[, .SD[, round(IQR(NPop))], by = 'region']
    
        
    p_sigma <- N_bnb[, .SD[, round(sd(NPop),2)], by = 'region']
    p_cv <- N_bnb[, .SD[, round(sd(NPop) / mean(NPop) * 100,2)], by = 'region']
    if(!is.null(ciprob)) {
        p_ci <- N_bnb[, .SD[, ci(NPop, ci = ciprob, method = method)], by = 'region']
    } else {
        p_ci <- N_bnb[, .SD[, ci(NPop, ci = 0.89, method = method)], by = 'region']
    }
