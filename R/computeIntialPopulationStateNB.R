...
#' @include utils.R
#' @export
computeInitialPopulationStateNB <- function(nnet, params, rndVal = FALSE, ciprob = NULL, method = 'ETI') {
    
    Ninit<-merge(nnet, params, by='region', all.x = TRUE, allow.cartesian = TRUE)

    N_stnb <- copy(Ninit)[, row := .I][
        , list(region = region,
               N = N,
               NPop = N + rnbinom(1, N + zeta + 1, 1 - beta * Q / (alpha + beta))), by = 'row']
    
    N_stnb[,row:=NULL]    
    pmean <- N_stnb[, .SD[, round(mean(NPop))], by = 'region']
    pmode <- N_stnb[, .SD[, round(Mode(NPop))], by = 'region']
    pmedian <- N_stnb[, .SD[, round(median(NPop))], by = 'region']
    pmin<- N_stnb[, .SD[, round(min(NPop))], by = 'region']
    pmax<- N_stnb[, .SD[, round(max(NPop))], by = 'region']
    pq1 <- N_stnb[, .SD[, round(as.numeric(quantile(NPop)[2]))], by = 'region']
    pq3 <- N_stnb[, .SD[, round(as.numeric(quantile(NPop)[4]))], by = 'region']
    piqr <- N_stnb[, .SD[, round(IQR(NPop))], by = 'region']
    
    p_sigma <- N_stnb[, .SD[, round(sd(NPop),2)], by = 'region']
    p_cv <- N_stnb[, .SD[, round(sd(NPop) / mean(NPop) * 100,2)], by = 'region']
    if(!is.null(ciprob)) {
        p_ci <- N_stnb[, .SD[, ci(NPop, ci = ciprob, method = method)], by = 'region']
    } else {
        p_ci <- N_stnb[, .SD[, ci(NPop, ci = 0.89, method = method)], by = 'region']
    }
